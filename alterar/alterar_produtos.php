<?php
session_start();
require_once '../conexao.php'; // aqui sua conexão deve instanciar PDO, ex: $pdo

// Verifica permissão do usuário (se usar controle de nível)
if ($_SESSION['nivel'] != 1) {
...
if (!isset($_POST['ID_produto'])) {
    echo "<script>alert('Produto não especificado!');window.location.href='../produtos.php'</script>";
    exit;
}

$id_produto = intval($_POST['ID_produto']);
$nome       = $_POST['Nome_prod'] ?? '';
$preco      = $_POST['Preco_unitario'] ?? 0;
$unidade    = $_POST['Unid_medida'] ?? '';
$quantidade = $_POST['Qntd_produto'] ?? 0;
$fornecedor = $_POST['ID_forn'] ?? '';
$categoria  = $_POST['id_categorias'] ?? '';
$validade   = $_POST['Validade'] ?? '';

// Converte validade para formato MySQL (YYYY-MM-DD)
if (!empty($validade)) {
